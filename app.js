...
function createHome(event) {
  ...
  saveSession({ role: "admin", memberName: adminUser });
...
function renderCreateMode() {
  createMode = "paid";
  $("createPricePill").textContent = "99 kr/mnd";
  $("createTitle").textContent = "Opprett kollektiv";
  $("createIntro").textContent = "Du blir admin og bruker ett av rommene. Medlemmer kommer inn via invitasjonslink.";
  $("paymentPanel").classList.remove("hidden");
  $("createSubmit").textContent = "Bekreft betaling og opprett";
  $("createBackButton").textContent = "Tilbake";
}
...
function joinHome(event) {
  ...
  const result = upsertMember(memberName, room);
  if (!result.ok) {
